    ...
    int nbytes, struct side_data *side)
{
    ...
    // ============================================================
    // HALO BONE CONDUCTION EQ (8kHz / 10ms Profile)
    // Cutoff target: ~150Hz to protect speakers
    // ============================================================

    // Safety check: ensure we actually have enough coefficients
    // For 8kHz 10ms, ns should be 80.
    if (ns >= 4) {
        // Bin 0 (0-50Hz): Deep rumble/DC drift.
        // Bone conduction can't reproduce this, just heat.
        xf[0] *= 0.001f;

        // Bin 1 (50-100Hz): Heavy bass thumps.
        // Causes "tickling" sensation on cheekbones.
        xf[1] *= 0.05f;

        // Bin 2 (100-150Hz): Upper bass.
        // Start blending back in.
        xf[2] *= 0.25f;

        // Bin 3 (150-200Hz): Low male vocals.
        // Almost back to normal level.
        xf[3] *= 0.80f;

        // Bins 4+ (200Hz+) are left at 1.0 (touched by original code only)
    }
